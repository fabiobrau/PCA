...
\begin{frame}{Dimensionality Reduction: A concrete example}
  ...
  \begin{minipage}[t]{0.5\textwidth}
    ...
    \end{figure}
  \end{minipage}
  \begin{block}{Theoretical Error Estimation}{}
    \begin{enumerate}
...
\begin{frame}{Eigen-pairs of Simmetric def. positive matrices}
  A matrix $A\in M(n)$ is symmetric and def.nneg if respectively 
  \begin{equation}
    A^TA=AA^T,\quad v^TAv\ge0\,\forall v\in\R^n
  \end{equation}
...
\begin{frame}{Approximation Error}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      \|f_{j,k-n+1}v_{n-k+1} + \cdots + f_nv_n\|^2 \\
      &=\frac{1}{N}\sum_{j=1}^N f_{j,k-n+1}^2+\cdots+f_{j,n}^2\\
